GeomVenn <- ggproto("GeomVenn", GeomPolygon,
                    ...
                    default_aes = aes(colour = "gray90", fill = NA, alpha = 0.5,
                                      size = 0.5, linetype = 1, fontface = "bold", family = "serif"),
...
#' @param set_name_face `string`, font face of set name text (default is `"bold"`)
#' @param set_name_family `string`, font family of set name text (default is `"serif"`)
...
#' @param set_total_face `string`, font face of set total text (default is `"bold"`).
#' @param set_total_family `string`, font family of set total text (default is `"serif"`).
...
#' @param count_face `string`, font face of segment count text (default is `"bold"`)
#' @param count_family `string`, font family of segment count text (default is `"serif"`).
...
#' segment be shown? (default is `FALSE`).
...
#' @param percentage_face `string`, font face of percentage text (default is `"bold"`).
#' @param percentage_family `string`, font family of percentage text (default is `"serif"`).
...
#'
#' # Use theme_void() to get a bold background
#' ggplot() +
#'  geom_venn(aes(set_name = set_name, elements = elements), data = df) +
#'  theme_void()
#'
...
#' lst2 <- gfun::data_read("genus.Syn.csv")
#' df2 <- prepare_venn(lst2, fill = c("blue", "green", "red", "black"))
#' ggplot() +
#'   geom_venn(aes(set_name = set_name, elements = elements, fill = fill),
#'            data = df2, type = "discrete") +
#'   theme_void()
